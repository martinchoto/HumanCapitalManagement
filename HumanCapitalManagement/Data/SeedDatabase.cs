    ...
    public class SeedDatabase
    {
        ...
        internal static void SeedUsersAndEmployees(ModelBuilder builder)
        {
            ...
            for (int i = 1; i <= 50; i++)
            {
                ...
                string country = SeedCountries()[random.Next(SeedCountries().Count)];
                string iban = CountryIbanGenerators[country]();
        ...
        {
            return CountryIbanGenerators.Keys.ToList();
        }
        private static string GenerateIban(string countryCode, string bankCode, string account)
        {
            string checkDigits = new Random().Next(10, 99).ToString();

            string bban = bankCode + account;
            return $"{countryCode}{checkDigits}{bban}".ToUpper();
        }
    }
}
